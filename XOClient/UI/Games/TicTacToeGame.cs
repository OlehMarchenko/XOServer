...
namespace XOClient.UI.Games
{
    public partial class TicTacToeGame : Form, IDisposable
    {
        ...
        {
            MessageBox.Show("Player left the game. You win!", "Game over", MessageBoxButtons.OK, MessageBoxIcon.Information);
            if (this.InvokeRequired)
            {
                this.Invoke(new Action(() =>
                {
        ...
        private void TicTacToeGame_FormClosing(object sender, FormClosingEventArgs e)
        {
            if (!win)
            {
                session.SendQuitGame(key);
            }
            this.session.IsConnection = false;
        }
